...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ########################single frame######################################
        # image_file = line["video_name"]+'_4.png'
        ...
        images=[]
        for i in range(4):
            image_file = line["video_name"] + '_%d.png'%(1+2*i)
            image = Image.open(os.path.join(self.image_folder, image_file)).convert('RGB')
            image_tensor_ = process_images([image], self.image_processor, self.model_config)[0]
            images.append(image_tensor_)
...
def eval_model(args):
    ...
    answer=json.load(open(os.path.expanduser(args.question_file.replace('q.json','a.json')), "r"))

    for i in range(len(questions)):
        assert questions[i]['question_id']==answer[i]['question_id']
        questions[i]['answer']=answer[i]['answer']
